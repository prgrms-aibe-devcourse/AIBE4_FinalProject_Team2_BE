...
public class AuthController {
    ...
    public ResponseEntity<?> signup(@RequestBody MemberDTO request) {
        ...
        if (memberRepository.existsByNickname(request.getNickname())) {
            return ErrorResponse.toResponseEntity(ErrorCode.AUTH_DUPLICATE_NICKNAME);
        }
    ...
    public ResponseEntity<?> reissue(
        ...
        if (refreshToken == null || refreshToken.isBlank()) {
            return ErrorResponse.toResponseEntity(ErrorCode.AUTH_TOKEN_NOT_FOUND);
        }

        // 1. Refresh Token 유효성 검사
        if (!jwtTokenProvider.validateToken(refreshToken)) {
            return ErrorResponse.toResponseEntity(ErrorCode.AUTH_TOKEN_EXPIRED);
        }
        ...
        if (!savedToken.getRefreshToken().equals(refreshToken)) {
            return ErrorResponse.toResponseEntity(ErrorCode.AUTH_TOKEN_INVALID);
        }
